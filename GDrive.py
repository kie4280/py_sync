...
class DriveClient:
    ...
    def __init__(self, cache_dir: str):
        ...
        self.folderQueries: queue.Queue = queue.Queue(100000)
        self.fileQueries: queue.Queue = queue.Queue(100000)
        ...
        self.__is_checking__: bool = False
        self.fileCounter_lock: threading.Lock = threading.Lock()
        self.folderCounter_lock: threading.Lock = threading.Lock()
        self.threadPoolExecutor = ThreadPoolExecutor(
            max_workers=DriveClient.max_threads)
        ...
        try:
            ...
            while self.terminate != True:
                ...
                try:
                    ...
                    for i in results:
                        id = i["id"]
                        parents: str = i["parents"][0]
                        path: str = self.folderPaths.get(
                            parents, "/")
                        self.folderPaths[id] = path + i["name"] + "/"
                        i["path"] = path
                    # self.folders.extend(results)
                    if first1:
                        first1 = False
                    else:
                        folderopen.write(",")
                    t: str = json.dumps(
                        results, ensure_ascii=False, separators=(",", ":"))[1:-1]
                    folderopen.write(t)
                except queue.Empty:
                    pass
                except Exception:
                ...
                try:
                    ...
                    for i in results:
                        id = i["id"]
                        parents: str = i["parents"][0]
                        path: str = self.folderPaths.get(parents, "/")
    ...
    def _onFileReceived(self, id, response):
        if self.terminate == False:
            ...
            c = len(results)
            self.fileCounter_lock.acquire(blocking=True)
            self.fileCount += c
            self.fileCounter_lock.release()
    ...
    def _listFiles(self, trashed: bool = False, **kwargs):
        ...
        while self.terminate == False and \
            (self.unfinished_file_req > 0 or self.unfinished_folder_req > 0
             or self.fileQueries.qsize() > 0 or self.folderQueries.qsize() > 0
             or self.searchFileQueue.qsize() > 0 or self.searchFolderQueue.qsize() > 0):
    ...
    def _onFolderReceived(self, id, response):
        if self.terminate == False:
            ...
            for f in results:
                ...
                f["isDir"] = True
            c = len(results)
            self.folderCounter_lock.acquire(blocking=True)
            self.folderCount += c
            self.folderCounter_lock.release()
            if c > 0:
    ...
    def _listFolders(self, trashed: bool = False, **kwargs):
        ...
        while self.terminate == False and \
            (self.unfinished_folder_req > 0 or self.searchFolderQueue.qsize() > 0 or
             self.folderQueries.qsize() > 0):
        ...
        while self.terminate == False and \
            ...
            for i in folder_futures:
                if i.done():
                    ...
                    if self.folder_threads + self.file_threads < DriveClient.max_threads - 1:
                        file_futures.append(
                            self.threadPoolExecutor.submit(self._listFiles))
                    folder_futures.remove(i)
